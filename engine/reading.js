...
// 일간(천간)별 쉬운 비유 — 짧고 쉽게
const ILGAN_EASY = {
  갑: { img: '큰 나무', desc: '곧고 진취적, 리더십이 있어요.' },
  을: { img: '풀·덩굴', desc: '부드럽고 유연하지만 끈기 있어요.' },
  병: { img: '태양', desc: '밝고 열정적, 표현을 잘해요.' },
  정: { img: '촛불', desc: '따뜻하고 섬세, 직관이 좋아요.' },
  무: { img: '큰 산', desc: '듬직하고 포용력이 커요.' },
  기: { img: '기름진 흙', desc: '섬세하고 실속 있게 가꿔요.' },
  경: { img: '쇠·도끼', desc: '결단력 있고 강직해요.' },
  신: { img: '보석·칼', desc: '예리하고 깔끔, 자존심이 있어요.' },
  임: { img: '강·바다', desc: '지혜롭고 생각이 깊어요.' },
  계: { img: '빗물·이슬', desc: '섬세하고 감수성이 풍부해요.' },
};
const EL_TRAIT = { 목: '성장·시작', 화: '열정·표현', 토: '안정·신뢰', 금: '결단·원칙', 수: '지혜·사색' };

// 오행이 부족할 때 — 어떤 사람인지·장단점·보완법 (쉽게)
...
// 상생(相生) 쌍이 둘 다 약할 때의 복합 설명
const SAENG_PAIR = {
  화토: '🔥+🏔 화생토(火生土) — 원래 화가 토를 만들어 주는데 둘 다 약하면: 에너지·추진력이 부족하고, 자신을 적극적으로 드러내지 않으며, 안정보다 변화·생각이 많아요. 아이디어는 많은데 실행·지속력이 약할 수 있고, 관계에서도 먼저 다가가기보다 관망하는 편이에요. 대신 감정 폭발이 적고 객관·이성적이며, 차분하게 분석하는 힘이 좋아요.',
  목화: '🌳+🔥 목생화(木生火) — 시작과 표현이 함께 약해, 의욕을 밖으로 드러내기보다 안으로 품는 편이에요. 대신 차분하고 무리하지 않아요.',
  토금: '🏔+⚔️ 토생금(土生金) — 안정과 마무리가 약해 끝맺음·관리가 느슨할 수 있어요. 대신 자유롭고 틀에 갇히지 않아요.',
  금수: '⚔️+💧 금생수(金生水) — 결단과 재충전이 약해 맺고 끊기·쉬어가기가 어려울 수 있어요. 대신 부드럽고 추진력이 있어요.',
  수목: '💧+🌳 수생목(水生木) — 사색과 시작이 약해 생각을 행동으로 잇기 더딜 수 있어요. 대신 솔직하고 유연해요.',
};

/**
 * 부족한 오행 풀이 — 어떤 사람인지·장단점·보완법.
 * @returns {{items: object[], combined: string[]}}
 */
export function buildElementWeakness(a) {
  const ELS = ['목', '화', '토', '금', '수'];
  const c = a.elements.count;
  let lacking = ELS.filter((e) => c[e] === 0);
  if (!lacking.length) lacking = ELS.filter((e) => c[e] === 1);
  if (!lacking.length) { const m = Math.min(...ELS.map((e) => c[e])); lacking = ELS.filter((e) => c[e] === m).slice(0, 1); }
  const items = lacking.map((el) => ({ el, ...ELEMENT_DETAIL[el] }));
  const edges = [['목', '화'], ['화', '토'], ['토', '금'], ['금', '수'], ['수', '목']];
  const combined = [];
...
export function buildSimpleSummary(saju, a, luck) {
  ...
  const headline = `${name}님은 “${saju.ilgan} ${el}” — ${ig.img} 같은 사람이에요.`;
  const lines = [];
  lines.push(`🌱 **성향** — ${ig.desc}`);

  const balLine = missing.length
    ? `${strong}(${EL_TRAIT[strong]}) 기운이 강하고, ${missing.join('·')}은 부족해요.`
  ...
  if (luck && luck.daewoon && luck.daewoon.currentIndex >= 0) {
    ...
    lines.push(`📅 **요즘** — ‘${cur.kor}’ 대운, ${cur.sipseong}의 시기예요. (참고용)`);
  }

  const adviceEl = missing[0] || strong;
  const adviceMap = {
    목: '새로운 일 하나 시작하기', 화: '마음을 말로 표현하기',
    토: '규칙적인 루틴 만들기', 금: '끊을 것 하나 정리하기', 수: '혼자 쉬는 시간 갖기',
  };
  lines.push(`💡 **오늘 한 걸음** — ${adviceMap[adviceEl] || '지금 감정 한 줄 적기'}.`);
